...
let mem1 = new Proxy({}, {
    set: (memory, addr, value) => {
        let valueAsBinary = value.toString(2).padStart(36, '0').split('');
        for (let i = 0; i < 36; ++i) {
            if (mask[i] == '0') {
                valueAsBinary[i] = '0';
            } else if (mask[i] == '1') {
        ...
        valueAsBinary = valueAsBinary.join('');
        let maskedValue = parseInt(valueAsBinary, 2);
        memory[addr] = maskedValue;
    }
});

console.log(execute(false, mem1));

function resolveFloating(addr, value, memory) {
    let idx = addr.indexOf('X');
    if (idx == -1) {
        let realAddr = parseInt(addr.join(''), 2);
        memory[realAddr] = value;
    } else {
        addr[idx] = '0';
        resolveFloating(addr, value, memory);
        addr[idx] = '1';
        resolveFloating(addr, value, memory);
        addr[idx] = 'X';
    }
...
let mem2 = new Proxy({}, {
    set: (memory, addr, value) => {
        let addrAsBinary = parseInt(addr).toString(2).padStart(36, '0').split('');
        for (let i = 0; i < 36; ++i) {
            if (mask[i] == '1') {
                addrAsBinary[i] = '1';
            } else if (mask[i] == 'X') {
        ...
        resolveFloating(addrAsBinary, value, memory);
    }
});

console.log(execute(false, mem2));
