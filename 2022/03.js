let data = `rNZNWvMZZmDDmwqNdZrWTqhJMhhgzggBhzBJBchQzzJJ
...
for (let line of data.split('\n')) {
    ...
    for (let i = 0; i < 52; ++i) {
        let l = ('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')[i];
        if (firstHalf.includes(l) && secondHalf.includes(l)) {
            sum += i + 1;
        }
    }
}
...
while (i < d.length) {
    ...
    let c = d[i+2];
    for (let l of L) {
        if (a.includes(l) && b.includes(l) && c.includes(l)) {
            sum += L.indexOf(l) + 1;
            break;
        }
    }
