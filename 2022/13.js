let data = `[[0,5,[[],[],2,[7,9]]],[[8,8,3,[6,3,8,9,1]],[2,0,10,7,10],4,10,[9,[1,8],4,[4,0,5,10],[4,0,8,8]]],[9,10],[],[0,[[]],4,10]]
...
function compareOrder(a, b) {
    if (typeof a === 'number' && typeof b === 'number') {
        ...
        if (a.length > 0 && b.length > 0) {
            let x = compareOrder(a[0], b[0]);
            if (x != 0) {
                return x;
            }
            return compareOrder(a.slice(1), b.slice(1));
...
for (let i = 0; i < lines.length; ++i) {
    let x = lines[i].split('\n');
    let r = compareOrder(JSON.parse(x[0]), JSON.parse(x[1]));
    if (r < 0) {
        sum += i + 1;
    } else if (r > 0) {
        continue;
    } else {
        console.log('ERROR lists are identical', lines);
    }
}
console.log(sum);
...
allpackets.sort((a, b) => compareOrder(JSON.parse(a), JSON.parse(b)));

let i1 = 1 + allpackets.indexOf('[[2]]');
let i2 = 1 + allpackets.indexOf('[[6]]');
console.log(i1 * i2);
