let data = `Game 1: 1 blue, 8 green; 14 green, 15 blue; 3 green, 9 blue; 8 green, 8 blue, 1 red; 1 red, 9 green, 10 blue
...
Game 100: 6 green, 15 red, 12 blue; 9 red; 16 red; 17 red, 3 blue, 7 green`.split('\n');
...
outer: for (let game of data) {
    let [, num, rounds] = game.match(/^Game (\d+): (.*)$/);
    num = parseInt(num);
    rounds = rounds.split('; ');
    for (let round of rounds) {
        let entries = round.split(', ');
        for (let entry of entries) {
            let [, count, type] = entry.match(/^(\d+) (\w+)$/);
            count = parseInt(count);
            if (type != 'green' && type != 'red' && type != 'blue') {
                console.log(num, 'skipping because of unknown type', type);
                continue outer;
            }
