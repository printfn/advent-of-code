let input = `bcj{s>2236:A,s<1879:A,x<3217:R,R}
...
function solve(input, part) {
    const [rulesStr, valuesStr] = input.split('\n\n');
    const values = valuesStr.split('\n').map(l => l.substring(1, l.length - 1).split(',').map(kv => {
        let [k, v] = kv.split('=');
        return [k, parseInt(v)];
    }).reduce((obj, [k, v]) => {
        obj[k] = v;
    ...
    const rules = rulesStr.split('\n').map(l => {
        const [, name, condsStr] = l.match(/^([a-z]+)\{(.*)\}$/);
        const condsParts = condsStr.split(',');
        const fallback = condsParts[condsParts.length - 1];
        const conds = condsParts.slice(0, condsParts.length - 1).map(c => {
            const [, k, op, v, nextRule] = c.match(/^([a-z]+)([<>])(\d+):([a-z]+|A|R)$/);
            return { k, op, v: parseInt(v), nextRule };
        });
        return { name, fallback, conds };
    });
    if (part === 1) {
        const process = obj => {
            let workflow = 'in';
            outer: while (true) {
                ...
                const rule = rules.find(r => r.name === workflow);
                for (let cond of rule.conds) {
                    if (cond.op === '<' && obj[cond.k] < cond.v
                        || cond.op === '>' && obj[cond.k] > cond.v) {

                        workflow = cond.nextRule;
                        continue outer;
                    }
                }
                workflow = rule.fallback;
            }
        };
        let sum = 0;
        ...
        const objToRange = obj => {
            let minObj = { x: 1, m: 1, a: 1, s: 1 };
            let maxObj = { x: 4000, m: 4000, a: 4000, s: 4000 };
            let workflow = 'in';
            let accept = false;
            outer: while (true) {
                if (workflow === 'R') {
                ...
                for (let cond of rule.conds) {
                    if (cond.op === '<') {
                        if (obj[cond.k] < cond.v) {
                            maxObj[cond.k] = Math.min(maxObj[cond.k], cond.v - 1);
                            workflow = cond.nextRule;
                            continue outer;
                        } else {
                            minObj[cond.k] = Math.max(minObj[cond.k], cond.v);
                        }
                    } else if (cond.op === '>') {
                        if (obj[cond.k] > cond.v) {
                            minObj[cond.k] = Math.max(minObj[cond.k], cond.v + 1);
                            workflow = cond.nextRule;
                            continue outer;
                        } else {
                            maxObj[cond.k] = Math.min(maxObj[cond.k], cond.v);
                        }
                    }
                }
                workflow = rule.fallback;
            }
            return { minObj, maxObj, accept };
        }
        let ranges = [objToRange({ x: 1, m: 1, a: 1, s: 1 })];
        rangeCount = ranges => ranges.reduce((sum, r) =>
            sum + 'xmas'.split('').reduce((p, k) => p * (r.maxObj[k] - r.minObj[k] + 1), 1), 0);
        while (rangeCount(ranges) < 4000 ** 4) {
            let randomObj = {
                x: randomInt(1, 4000),
                m: randomInt(1, 4000),
                a: randomInt(1, 4000),
            ...
            while (ranges.some(r => overlap(r.minObj)))
            if (ranges.some(r => 'xmas'.split('').every(k =>
                    r.minObj[k] <= randomObj[k] && randomObj[k] <= r.maxObj[k]))) {
                continue;
            }
            console.log(rangeCount(ranges) / 4000 ** 4, rangeCount(ranges.filter(r => r.accept)));
            const newRange = objToRange(randomObj);
            let o = ranges.filter(r => 'xmas'.split('').every(k => overlap(r.minObj[k], r.maxObj[k], newRange.minObj[k], newRange.maxObj[k])));
            if (o.length !== 0) {
                console.log('OVERLAP', randomObj, newRange, o);
                return;
            }
            ranges.push(newRange);
        }
        console.log(rangeCount(ranges) / 4000 ** 4);
        console.log(rangeCount(ranges.filter(r => r.accept)));
    }
}
