...
let data = `F|J-F|.|.F-F-7-7F--|77F.7.77F|-L7-|7|-|-F-L7.7FJF-7-JF7.77-F.7F7.FLF7-FF-L7F--F-J-..J-FJ-FLF7.FF.-F7-|-.F7--77F---7-LF7FFL7.F7|7FJ-FL-.-FJ-F
...
function ispipe(ch) {
    return '|-LJ7F'.includes(ch);
}

function adjacent(data, x, y) {
    switch (data[y][x]) {
    ...
    throw new Error(`Unknown pipe ${ch} at ${x},${y}`);
}

function valid(data, pos) {
    return pos[0] >= 0 && pos[0] < data[0].length && pos[1] >= 0 && pos[1] < data.length && ispipe(data[pos[1]][pos[0]]);
}

function isloop(data, pos) {
    if (!valid(data, pos))
        return null;
    let [prev, next] = adjacent(data, pos[0], pos[1]);
    if (!valid(data, prev) || !valid(data, next))
        return null;
    let loopPositions = [];
    let posSet = new Set();
    while (true) {
        let [a, b] = adjacent(data, next[0], next[1]);
        if (!valid(data, a) || !valid(data, b)) return null;
        if (!a.equals(pos) && !b.equals(pos)) return null;
        if (b.equals(pos)) {
            [a, b] = [b, a];
        }
        pos = next;
        ...
        if (posSet.has(str)) break;
        loopPositions.push(next);
        posSet.add(str);
    }
    return loopPositions;
}

function findLoop(data, ch) {
    ...
    for (let y = 0; y < data2.length; ++y) {
        for (let x = 0; x < data2[y].length; ++x) {
            ...
            let loop = isloop(data2, [x, y]);
            if (loop) {
                let containsS = loop.some(p => p.equals(s));
                if (!containsS) continue;
                return { loop, s, S: ch };
            }
        }
    }
...
function solve1(data, s) {
    data = data.split('\n').map(l => l.split(''));
    return findLoop(data, s).loop.length / 2;
}

// console.log(solve1(sample, 'F'));
console.log(solve1(data, '7'));

function solve2(data, s) {
    data = data.split('\n').map(l => l.split(''));
    let loop = findLoop(data, s).loop;
    let loopSet = new Set();
    for (let entry of loop) {
        loopSet.add(`${entry[0]},${entry[1]}`);
    }
    let area = 0;
...
// console.log(solve2(sample2, 'F'));
console.log(solve2(data, '7'));