...
for (let entry of data) {
    let digit1 = entry.match(/(\d)/)[1];
    let digit2 = entry.match(/.*(\d).*?$/)[1];
    let combined = parseInt(digit1 + digit2);
    sum += combined;
}
console.log(sum);
...
sum = 0;
for (let entry of data) {
    let digit1 = entry.match(/(\d|one|two|three|four|five|six|seven|eight|nine)/)[1];
    let digit2 = entry.match(/.*(\d|one|two|three|four|five|six|seven|eight|nine).*?$/)[1];
    let combined = parseInt('' + convert(digit1) + convert(digit2));
