let data = `................458...689.556..3............197......582........720.........................515..352..286.........670.741.....895.626.......
...
.....................815.........786................186...........640......388.......415....4..36.....894.........................303.......`.split('\n');
...
function get(x, y) {
    if (x < 0 || y < 0) return;
    if (x >= data.length || y >= data[x].length) return;
    return data[x].split('')[y];
}
...
for (let i = 0; i < data.length; ++i) {
    ...
    for (let j = 0; j < row.length; ++j) {
        ...
        if (j > 0 && /\d/.test(row[j-1])) continue;
        let numberStr = ch;
        for (let k = j+1; k < row.length && /\d/.test(row[k]); ++k) numberStr += row[k];
        let number = parseInt(numberStr);
        let valid = false;
        let adjacentCoords = [
            [i, j-1],
            [i, j + numberStr.length],
            [i-1, j-1],
            [i-1, j+numberStr.length],
            [i+1, j-1],
            [i+1, j+numberStr.length]];
        for (let k = 0; k < numberStr.length; ++k) {
            adjacentCoords.push([i-1, j+k]);
            adjacentCoords.push([i+1, j+k]);
        }
        for (let [x,y] of adjacentCoords) {
            valid |= getValid(x, y);
        }
        for (let [x,y] of adjacentCoords) {
            if (get(x, y) === '*') {
                const idxStr = `${x},${y}`;
                if (!gearNumbers[idxStr]) {
                    gearNumbers[idxStr] = [];
                }
                gearNumbers[idxStr].push(number);
            }
        }
        if (valid) {
            sum += number;
...
console.log(sum);
let sum2 = 0;
for (let key in gearNumbers) {
    if (gearNumbers[key].length !== 2) continue;
    sum2 += gearNumbers[key][0] * gearNumbers[key][1];
}
console.log(sum2);
