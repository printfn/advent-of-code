...
["jrcczb xfvtzq nmszz sbsrk dpkdp pdxhgx cgrcmp qbnkj mbdksj rkzqs scqc rxzxs xzhct mkpxx ttr qglps vzzxl xlqj jsdcr pbtns qzbzd fvvjp vllxj zjsz frnh gnrb vpl zdrlf gztf vrpzss vrzkz dmpl hphcb gbzcv rgt kxl ctmzsr nlxh zjsh lvjrbms sltl brhfcbt lsc pmhnmxr fhpxz qsrh vkjggtx zcxxdp drpxzr jgxbzks", ["peanuts"]],
["tcp vlqtzzl zjsh rkzqs drpxzr cqbkj ftfx gbzcv xvmftv fptpk rncjx rnxl jtfnm nqstv znfrgvc jsdcr zmhnj jrnfp qfnq hphcb vzzxl bbkth flhzd jgxbzks ssgkt xcdb fvpj rlxhc ztrpkb qsrh qbtpnr bqfqcjb xzhct pznl hdnq qlt tjxp lvjrbms fhpxz cpj dpkdp mbdksj qpgj vrzkz xrlmvz rvnfn snxz hdkc ltxqz zjsz hxfrh pmhnmxr", ["nuts", "fish", "peanuts"]],
["vrtfrj ctmzsr zpbbvvm nvdst qvhdl rlxhc pbtns tcp lsc vrzkz dqdn hmshx fptpk rph rgt vllxj dbfk dkxr ttr nmszz qvhft znjp fhpxz jgv ftfx zjdzx zjsh znfrgvc qdbj vkjggtx znklc lhsgbq jsdcr frz pspjgzx pkmbk vpl pkgsf vfnv dnzljk hqlm klfp zmhnj njcjm pdxhgx dlc cntfbt ghtjbp qpgj rkzqs vtrltkq jrcczb fzgbq fjbjj vzzxl cgrcmp lvjrbms vlqtzzl hphcb svrx dmpl dxpdkp pjgxpg bbkth ztlbvp rncjx cqbkj", ["shellfish", "eggs", "soy"]],
["spzmvfl bccz xzhct qdbj qllxhr jgv rkzqs slglnd qlt tjxp rncjx rnxl fvpj fjbjj zhvsdc qnt nvbr mbdksj tkrr hphcb rcv znklc qdbq fzgbq nlxh rph rlxhc msvgj lhsgbq dfpqn cqbkj vrzkz znfrgvc tqtbd cntfbt zmhnj vrpzss xrlmvz flhzd zjsh httx znjp nmszz pbmhl pspjgzx ctmzsr kxl ftfx qzbzd zpbbvvm scqc jtfnm njcjm fvvjp", ["soy", "shellfish", "fish"]],
["brhns bbkth jtfnm znklc xzhct jrnfp tcp fptpk cqbkj mkpxx vpl ssgkt jgkp rmthpj hxfrh fvvjp zjsh pkgsf fzpds zmhnj tqtbd ltxqz rkzqs pmhnmxr rnxl gztf qvhft rxzxs qsrh ctmzsr fvd sks qbnkj xrlmvz rzjcs zmf hbvqn qbtpnr mbdksj nlxh nvdst ghtjbp slmjchql hphcb lhb zvmnv xcdb vllxj hmshx znjp vrpzss dbfk pchhkp klfp lqtnn bxhsq slglnd vlqtzzl cvz zdrlf vrzkz cpsbgsh nvhnsg pkmbk fzgbq dfpqn dlc fmkbnr cvtj", ["dairy"]],
["jtzsqznl bhlbm lsc lhb zpbbvvm ftfx znjp hphcb nqstv zjsh dksfbk pchhkp fzgbq pmhnmxr vrpzss knhvxm mbdksj pbmhl rkzqs httx bqfqcjb qvhft lqdpl jrlvkz zmhnj sks qbnkj nvdst czlpq pznl tnjxcf tkrr vrzkz hkzzg cf zdrlf nfzczn jrnfp gmx spzmvfl vzzxl rnxl znfrgvc dqdn jgxbzks jtfnm xcdb", ["nuts"]],
];
...
for (let f of foods) {
    for (let a of f.allergens) {
        if (!allAllergens.has(a)) {
            allAllergens.add(a);
        }
    }
    for (let i of f.ingredients) {
        if (!allIngredients.has(i)) {
...
for (let allergen of allAllergens) {
    ...
    for (let food of foods) {
        if (food.allergens.includes(allergen)) {
            if (ingredients == null) {
                ingredients = JSON.parse(JSON.stringify(food.ingredients));
            } else {
                ingredients = ingredients.filter(ing => food.ingredients.includes(ing));
            }
        }
    }
    // Uncomment this to work out part 2
    //console.log(allergen, ingredients);
    for (let uns of ingredients) {
        if (!unsafe.has(uns)) {
            unsafe.add(uns);
        }
    }
...
nuts [ 'mbdksj', 'zjsh' ]
shellfish [ 'rkzqs' ]
soy [ 'rkzqs', 'zmhnj', 'ctmzsr' ]
eggs [ 'rkzqs', 'zjsh', 'vzzxl', 'zmhnj', 'ctmzsr' ]
sesame [ 'rkzqs', 'ctmzsr' ]
fish [ 'rkzqs', 'hphcb' ]
dairy [ 'rkzqs', 'zjsh', 'zmhnj', 'vrzkz' ]
peanuts [ 'rkzqs', 'hphcb', 'vzzxl' ]
*/
...
console.log('vrzkz,zjsh,hphcb,mbdksj,vzzxl,ctmzsr,rkzqs,zmhnj');
