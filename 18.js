...
for (let expr of exprs) {
    let newExpr = '(' + expr + ')';
    for (let i = 0; i < 40; ++i) {
        newExpr = newExpr.replace(/\(\d+ [+*] \d+/, (match) =>  {
            return '(' + eval(match + ')').toString();
        });
        newExpr = newExpr.replace(/\((\d+)\)/, (match, $1) =>  {
            return $1;
        });
    }
    sum += parseInt(newExpr);
}
console.log(sum);
...
for (let expr of exprs) {
    let newExpr = '(' + expr + ')';
    for (let j = 0; j < 10; ++j) {
        newExpr = newExpr.replace(/\((\d+( [+*] \d+)+)\)/, (match, $1) =>  {
            let newExpr = $1;
            for (let i = 0; i < 10; ++i) {
                newExpr = newExpr.replace(/(\d+) \+ (\d+)/, (match, $1, $2) =>  {
                    return (parseInt($1) + parseInt($2)).toString();
                });
            }
            for (let i = 0; i < 10; ++i) {
                newExpr = newExpr.replace(/(\d+) \* (\d+)/, (match, $1, $2) =>  {
