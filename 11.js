const initialState = ["LLLLLLLLLL.LLLL.LLLLLLL.LLLLL.LLLLLLLLLLLLLLLLLLLLLL.LL.LLLLLLLLLLL.LLLLLL.LL.LLLL.LLLLLL.LLLLLLL",
...
let countAdj = (state, x, y) => {
    let count = 0;
    if (x > 0 && state[x - 1][y] == '#') count++;
    if (x > 0 && y > 0 && state[x - 1][y - 1] == '#') count++;
    if (y > 0 && state[x][y - 1] == '#') count++;
    if (x < state.length - 1 && y > 0 && state[x + 1][y - 1] == '#') count++;
    if (x < state.length - 1 && state[x + 1][y] == '#') count++;
    if (x < state.length - 1 && y < state[0].length - 1 && state[x + 1][y + 1] == '#') count++;
    if (y < state[0].length - 1 && state[x][y + 1] == '#') count++;
    if (x > 0 && y < state[0].length - 1 && state[x - 1][y + 1] == '#') count++;
    return count;
}

let step = (state, countFn, threshold) => {
    let next = [];
    for (let x = 0; x < state.length; ++x) {
        ...
        for (let y = 0; y < state[x].length; ++y) {
            switch (state[x][y]) {
                case '.': row.push('.'); break;
                case 'L': if (countFn(state, x, y) == 0) { row.push('#'); } else { row.push('L'); } break;
                case '#': if (countFn(state, x, y) >= threshold) { row.push('L'); } else { row.push('#'); } break;
            }
        }
        next.push(row);
    }
...
let countTotalOccupied = (state) => {
    let totalOccupied = 0;
    for (let x = 0; x < state.length; ++x) {
        for (let y = 0; y < state[x].length; ++y) {
            if (state[x][y] == '#') totalOccupied++;
        }
    }
    return totalOccupied;
};

let countOccupiedWithFn = (initialState, countFn, threshold) => {
    let state = step(initialState, countFn, threshold);
    let nextState = step(state, countFn, threshold);
    while (!equal(state, nextState)) {
        state = nextState;
        nextState = step(state, countFn, threshold);
    }

    return countTotalOccupied(state);
}

console.log(countOccupiedWithFn(initialState, countAdj, 4));

let isOccupiedInDirection = (state, x, y, dx, dy) => {
    do {
        x += dx;
        y += dy;
    } while (x >= 0 && y >= 0 && x <= state.length - 1 && y <= state[0].length - 1 && state[x][y] == '.');
    return x >= 0 && y >= 0 && x <= state.length - 1 && y <= state[0].length - 1 && state[x][y] == '#';
}

let countFirstVisible = (state, x, y) => {
    let count = 0;
    if (isOccupiedInDirection(state, x, y, 1, 0)) count++;
    if (isOccupiedInDirection(state, x, y, 1, 1)) count++;
    if (isOccupiedInDirection(state, x, y, 0, 1)) count++;
    if (isOccupiedInDirection(state, x, y, -1, 1)) count++;
    if (isOccupiedInDirection(state, x, y, -1, 0)) count++;
    if (isOccupiedInDirection(state, x, y, -1, -1)) count++;
    if (isOccupiedInDirection(state, x, y, 0, -1)) count++;
...
console.log(countOccupiedWithFn(initialState, countFirstVisible, 5));
