...
async function runChallenge(challenge: Challenge, year: number, day: number) {
    ...
    for (const run of runs) {
        ...
        const duration = `\x1b[2m${(endTime - startTime).toFixed(3).padStart(10)} ms\x1b[0m`;
        if (actual === expected) {
            console.log(`  ✅ Correct  ${actual} ${duration}`);
        } else {
            console.log(`  ❌ Incorrect  expected: ${expected} actual: ${actual} ${duration}`);
        }
    }
}
