...
function readmem(input, idx, mode, relbase) {
    switch (mode) {
        case 0:
            return input[idx] || 0;
        case 1:
            return idx;
        case 2:
            return input[relbase + idx] || 0;
    }
}

function writemem(input, idx, value, mode, relbase) {
    switch (mode) {
        case 0:
            input[idx] = value;
            break;
...
function execute(input, inputfn, outputfn) {
    ...
    while (true) {
        ...
        switch (opcode % 100) {
            case 1: { // add
                let a = readmem(input, input[pc + 1], modeA, relbase);
                let b = readmem(input, input[pc + 2], modeB, relbase);
                let res = a + b;
                if (isNaN(a) || isNaN(b)) throw new Error('nan');
                writemem(input, input[pc + 3], res, modeC, relbase);
                pc += 4;
                break;
            }
            case 2: { // mul
            ...
            case 3: { // read input
                ...
                writemem(input, input[pc + 1], val, modeA, relbase);
                pc += 2;
                break;
            }
            case 4: { // write output
                outputfn(readmem(input, input[pc + 1], modeA, relbase));
                pc += 2;
                break;
            }
            case 5: { // jump-if-true
            ...
            case 7: { // less than
                ...
                let b = readmem(input, input[pc + 2], modeB, relbase);
                if (isNaN(a) || isNaN(b)) throw new Error('nan');
                writemem(input, input[pc + 3], a < b ? 1 : 0, modeC, relbase);
                pc += 4;
                break;
            }
            case 8: { // equals
            ...
            case 9: { // relative base offset
                let offset = readmem(input, input[pc + 1], modeA, relbase);
                relbase += offset;
                pc += 2;
                break;
            }
...
//execute([109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99], () => 1, console.log);
//execute([1102,34915192,34915192,7,4,7,99,0], () => 1, console.log);
//execute([104,1125899906842624,99], () => 1, console.log);
