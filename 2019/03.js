...
function followWire(wire, f) {
    let x = 0;
    let y = 0;
    let steps = 0;
    for (let ins of wire) {
        let direction = ins[0];
        let amount = parseInt(ins.substring(1));
...

{
    ...
    followWire(wire1, (x, y, steps) => positions.add(x + ',' + y));

    let intersections = [];
    followWire(wire2, (x, y, steps) => {
    ...
    intersections.sort(([x1, y1], [x2, y2]) => {
        let d1 = Math.abs(x1) + Math.abs(y1);
        let d2 = Math.abs(x2) + Math.abs(y2);
        return d1 < d2 ? -1 : 1;
    });

    let [x, y] = intersections[0];
...

{
    let positions = new Set();
    let w1distances = {};
    followWire(wire1, (x, y, steps) => {
        let posString = x + ',' + y;
        positions.add(posString);
        w1distances[posString] = steps;
    });
    ...
    followWire(wire2, (x, y, steps) => {
        ...
        if (positions.has(posString)) {
            if (!(x == 0 && y == 0)) {
                let stepSum = steps + w1distances[posString];
                intersections.push(stepSum);
            }
        }
    });
