declare module "a-star" {
	interface Options<T> {
		 ...
		 * Equivalently, for every node `x` and successor `y` of `x`, the
		 * estimated cost of reaching the goal from `x` must be no greater than
		...
		/** Limit the number of milliseconds to search before returning null. */
